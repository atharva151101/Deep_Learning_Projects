...
class DatasetReader1(torch.utils.data.Dataset):
    ...
    def __init__(self, file_path, mode,num):
        #self.transform = transforms.Compose([transforms.ToTensor()])
        self.num=num
        self.cols = ["x{i}".format(i= i) for i in range(num)]
...
class DatasetReader3(torch.utils.data.Dataset):
    ...
    def __init__(self, file_path, mode):
        ...
        X_train, X_val, y_train, y_val = train_test_split(X, y, train_size = 0.8, random_state = 420)
       # X_val, Xtest, y_val, ytest = train_test_split(X_val, y_val, train_size = 0.01, random_state = 42)
        if mode == 'train':
            self.X = X_train.reset_index(drop = True)
            self.y = y_train.reset_index(drop = True)
